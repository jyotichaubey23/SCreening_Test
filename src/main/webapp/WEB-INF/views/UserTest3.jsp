...
<body style="font-family: Poppins-Regular;">
			...
			<div class="row login">
				...
				<div class="col-md-12  mx-auto m-5">
					<div class="jumbotron">
						...
						<!-- To display correct wrong image -->
						<%-- 	<div class ="correctWrongSign">
									<div class = "d-inline" id ="tickImage">
									 <img src="${pageContext.request.contextPath}/images/check.png" width="30" height="30">
									</div>
									
									<div  class = "d-inline" id ="crossImage">
									 <img src="${pageContext.request.contextPath}/images/remove.png" width="30" height="30">
									</div>
									</div> --%>
 ...
 -->
	...
	<form action="${pageContext.request.contextPath}/testgiven3"
	 ...
	 <div class="form-row">
		...
		<div id="test"></div>
		<input type='hidden' id="score" name="module3_score" value='' />
		<!--   <form action='' id='form1' method='POST'  
        onsubmit='javascript:checkAnswer();'> -->
		<div class="text-center">
			<BUTTON type="submit" class="btn btn-danger" id="submit">Submit
				Test</BUTTON>
		</div>
...
var arrSum=0;	
var pos = 0,previousPos=0, test, test_status, question, choice, choices, chA, chB, chC, correct = 0;
var questions = [
...
function renderQuestion(){
	...
	if(pos >= questions.length){
		test.innerHTML = "";
		_("test_status").innerHTML = "<h2>Thankyou, Module 3 is complete";
		/* pos = 0; */
/* 		correct = 0; */
		return false;
	}
...
function previousQues()
{
	...
	for(var i=0; i<choices.length; i++){
       ...
       //     alert(map[pos]);   // display previous option tracking
		}
	}
...
{
	//alert("Entered correctScore loop");
	 arrSum=0;
	...
	correct = arrSum;
/* 	var total=0;
	for(var k in indexCorrect) { total += arr[k]; }
	alert("arrSum : "+total); */
}
...
function checkAnswer(){
	...
	for(var i=0; i<choices.length; i++){
		if(choices[i].checked){
            ...
            // to keep a track and display green color of attempted questions
...
//	correct++;                         //to correct this correct use an array and add all in the end 1 for correct and 0 for wrong.
	//alert ("correct" + correct);     
	
	// 1 is for correct option and 0 is for wrong option
	if(indexCorrect[pos] == 0 || indexCorrect[pos] == null )
		{
		indexCorrect[pos]=1;
		//alert("entered if indexCorrect[pos]:"+indexCorrect[pos]  );
	
		// toggle control between corrent and incorrect
		/* document.getElementById('tickImage').style.visibility = 'visible';
		document.getElementById('crossImage').style.visibility = 'hidden'; */
		}
}
else
	{
	indexCorrect[pos]=0;
	//alert("entered else indexCorrect[pos]:"+indexCorrect[pos] );
	
	/* document.getElementById('crossImage').style.visibility = 'visible';
	document.getElementById('tickImage').style.visibility = 'hidden'; */
	}
...
calculateCorrectScore();
    ...
    // to keep a track and display questions as we move ahead hitting the next question button
    // all the buttons are green by default
...
var sec = 60, countDiv = document.getElementById("timer"), secpass, countDown = setInterval(
		function() {
			'use strict';
			secpass();
		}, 1000);
...
function secpass() {
	...
	if (sec > 0) {
		...
		window.location.replace("jsp/TestModules.jsp");
 ...
 */
 ...
 navigator.mediaDevices.getUserMedia({
	...
	<!--   Boolean to check if 2nd time tab opened as first time we'll issue a warning
 -->
...
if(document.visibilityState == "hidden")
{
	...
	flagTabClose = true;
   //alert("Please refrain from using unfair means, next time you'll be logged out");
